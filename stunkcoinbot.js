...
client.on("ready", () => {
  console.log(
    "Stunkcoin is ready and online! Created by abkelol on (Fiverr) ingore that, fuck Fiverr."
  );
...
for (const module1 of modules) {
  const commandFiles = fs
    ...
    .filter(file => file.endsWith(".coffee") || file.endsWith(".js"));
...
client.on("message", message => {
  ...
  if (
    command.ownerOnly &&
    message.author.id != 445234723590242304 &&
    message.author.id != 362806280458469376
  )
  ...
  if (timestamps.has(message.author.id)) {
    ...
    if (now < expirationTime) {
      return message.reply(
        `Please wait for ${days} day(s), ${hours} hour(s), ${minutes} minute(s), ${seconds} second(s) before reusing the \`${command.name}\` command.`
      );
    }
  }
